...
from teleop.robot_control.upper_body_retarget import UpperBodyRetarget
...
from teleop.robot_control.robot_hand_inspire_grip1 import Inspire_Grip1_Controller
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ee', type=str, choices=['dex1', 'dex3', 'inspire1', 'inspire1_grip1', 'brainco'], help='Select end effector controller')
    # mode flags
    ...
    try:
        ...
        if WRIST and args.sim:
            # In simulation, the server concatenates head + left wrist + right wrist + world_camera.
            # tv_img_array stores head; wrist_img_array stores the remaining 3 views (left, right, world).
        ...
        # upper body retarget (waist from head)
        upper_body = UpperBodyRetarget(height=1.70, waist_height_ratio=0.530)
        ...
        elif args.ee == "inspire1_grip1":
            left_gripper_value = Value('d', 0.0, lock=True)        # [input]
            right_gripper_value = Value('d', 0.0, lock=True)       # [input]
            dual_gripper_data_lock = Lock()
            dual_gripper_state_array = Array('d', 2, lock=False)   # [output] current left, right grip state(2) data.
            dual_gripper_action_array = Array('d', 2, lock=False)  # [output] current left, right grip action(2) data.
            hand_ctrl = Inspire_Grip1_Controller(
                left_gripper_value,
                right_gripper_value,
        ...
        while not STOP:
            ...
            # compute waist q from head pose and send to controller (G1_29)
            q_upper_body = None
            if tele_data.head_pose is not None and args.arm == "G1_29":
                q_upper_body = upper_body.solve_upper_body_angles(tele_data.head_pose)
                arm_ctrl.ctrl_upper_body(q_upper_body)
            ...
            elif args.ee == "inspire1_grip1" and args.xr_mode == "hand":
                # Map thumb-index pinch distance to scalar grip command in [0, 1].
                with left_gripper_value.get_lock():
                    left_gripper_value.value = tele_data.left_pinch_value
                with right_gripper_value.get_lock():
                    right_gripper_value.value = tele_data.right_pinch_value
            ...
            if args.record:
                ...
                if args.ee == "dex3" and args.xr_mode == "hand":
                    with dual_hand_data_lock:
                        ...
                        # record waist state/action when using hand teleop
                        if args.arm == "G1_29":
                            all_q = arm_ctrl.get_current_motor_q()
                            current_body_state = [
                                float(all_q[G1_29_JointIndex.kWaistYaw]),
                                float(all_q[G1_29_JointIndex.kWaistPitch]),
                                float(all_q[G1_29_JointIndex.kWaistRoll]),
                            ]
                            current_body_action = (
                                q_upper_body.tolist() if q_upper_body is not None else []
                            )
                        else:
                            current_body_state = []
                            current_body_action = []
                ...
                if RECORD_RUNNING:
                    colors = {}
                    depths = {}
                    # Save robot-view head feed as usual.
                    if BINOCULAR:
                        # Stereo head: split into left/right views.
                    ...
                    # Add wrist and world-camera images if available.
                    if WRIST:
                        try:
                            if args.sim:
                                # Simulation: wrist_img_array packs [left_wrist, right_wrist, world_camera].
                                num_segments = 3
                                seg_w = wrist_img_shape[1] // num_segments
                                left_wrist_img = current_wrist_image[:, 0:seg_w]
                                right_wrist_img = current_wrist_image[:, seg_w : 2 * seg_w]
                                world_img = current_wrist_image[:, 2 * seg_w : 3 * seg_w]

                                colors[f"color_{next_color_idx}"] = left_wrist_img
                                colors[f"color_{next_color_idx + 1}"] = right_wrist_img
                                colors[f"color_{next_color_idx + 2}"] = world_img
                            else:
                                # Real robot: wrist_img_array packs [left_wrist, right_wrist].
