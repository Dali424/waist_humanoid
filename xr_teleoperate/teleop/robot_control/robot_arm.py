...
class G1_29_ArmController:
    ...
    def _ctrl_motor_state(self):
        ...
        while True:
            ...
            self.msg.motor_cmd[G1_29_JointIndex.kWaistYaw].q   = waist_q_target[0]
            self.msg.motor_cmd[G1_29_JointIndex.kWaistRoll].q  = waist_q_target[2]  # stored as [yaw, pitch, roll]
            self.msg.motor_cmd[G1_29_JointIndex.kWaistPitch].q = waist_q_target[1]
    ...
    def ctrl_upper_body(self, q_upper_body):
        '''Set control target [yaw, pitch, roll] for waist joints.'''
        with self.ctrl_lock:
            # ensure 3 elements; map to [yaw, pitch, roll]
            if isinstance(q_upper_body, (list, tuple, np.ndarray)) and len(q_upper_body) == 3:
                self.waist_q_target = np.array(q_upper_body, dtype=float)
            else:
