...
parser.add_argument("--viewport_camera", type=str, default="", help="Viewport camera name or USD prim path (e.g., front_camera, left_wrist_camera, right_wrist_camera or /World/... path)")
...
class _ResetKeyListener:
    """Lightweight keyboard listener for in-sim resets.

    - Press '9' to reset only task objects (spawn area logic).
    - Press '0' to reset the whole scene to defaults.

    Falls back gracefully if pynput is unavailable (e.g., headless CI).
    """
    ...
    def start(self):
        try:
            ...
            def on_press(key):
                try:
                    ch = key.char if hasattr(key, 'char') and key.char else None
                except Exception:
                    ch = None
                if ch == '9':
                    with self._lock:
                        self._pending = 'object'
                    print('[keys] 9 pressed -> request object reset')
                elif ch == '0':
                    with self._lock:
                        self._pending = 'all'
                    print('[keys] 0 pressed -> request full reset')

            self._listener = keyboard.Listener(on_press=on_press)
            self._listener.daemon = True
            self._listener.start()
            self.enabled = True
            print('[keys] Keyboard reset enabled (9=object, 0=all)')
        except Exception as e:
            # Keyboard input not available; continue without it
            self.enabled = False
            print(f"[keys] Keyboard reset disabled: {e}")
...
def main():
    ...
    try:
        ...
        try:
            ...
            # If requested, set main viewport to look through a robot camera
            if args_cli.viewport_camera and not args_cli.no_render:
                def _set_viewport_to_camera(camera_hint: str):
                    try:
                        import omni.usd
                        stage = omni.usd.get_context().get_stage()
                        hint = camera_hint.strip()
                        candidates = []
                        if hint.startswith("/World/"):
                            candidates.append(hint)
                        else:
                            h = hint.lower()
                            name_sets = {
                                "front_camera": ["front_camera", "front_cam", "PerspectiveCamera_robot"],
                                "left_wrist_camera": ["left_wrist_camera"],
                                "right_wrist_camera": ["right_wrist_camera"],
                            }
                            base = name_sets.get(h, [hint])
                            candidates.extend(base)
                        sel_path = None
                        # Prefer cameras under env_0
                        if stage:
                            from pxr import Usd
                            for prim in stage.Traverse():
                                try:
                                    ...
                                    if "/World/envs/env_0/" not in p_str:
                                        continue
                                    name = prim.GetName()
                                    if any(name == c or name.endswith(c) or p_str.endswith(c) for c in candidates):
                                        sel_path = p_str
                                        break
                                except Exception:
                                    continue
                            if not sel_path:
                                for prim in stage.Traverse():
                                    if prim.GetTypeName() == "Camera" and "/World/envs/env_0/" in str(prim.GetPath()):
                                        sel_path = str(prim.GetPath())
                                        break
                        if not sel_path:
                            print(f"[camera] viewport camera not found for hint '{camera_hint}'")
                            return
                        # Set viewport active camera (utility or legacy APIs)
                        try:
                            import omni.kit.viewport.utility as vp_utils
                            vp = vp_utils.get_active_viewport()
                            if hasattr(vp, "camera_path"):
                                vp.camera_path = sel_path
                                print(f"[camera] viewport set to {sel_path} (utility)")
                                return
                        except Exception:
                            pass
                        try:
                            ...
                            win = vp_iface.get_viewport_window()
                            if win:
                                win.set_active_camera(sel_path)
                                print(f"[camera] viewport set to {sel_path} (legacy)")
                                return
                        except Exception:
                            pass
                    except Exception as e:
                        print(f"[camera] failed to set viewport camera: {e}")
                _set_viewport_to_camera(args_cli.viewport_camera)
        except Exception as e:
    ...
    if not args_cli.replay_data:
        ...
        # start keyboard reset listener (optional)
        reset_keys = _ResetKeyListener()
        reset_keys.start()
    else:
        print("========= create dds =========")
        try:
        ...
        # in replay mode, still allow key resets if available
        reset_keys = _ResetKeyListener()
        reset_keys.start()
        ...
        with contextlib.suppress(KeyboardInterrupt), torch.inference_mode():
            while simulation_app.is_running() and controller.is_running:
                ...
                if not args_cli.replay_data:
                    ...
                    # Handle DDS-driven reset commands
                    if reset_pose_cmd is not None:
                        try:
                            ...
                            raise e
                    # Handle keyboard-driven reset requests
                    try:
                        req = reset_keys.pop()
                        if req == 'object':
                            print("[keys] reset object")
                            env_cfg.event_manager.trigger("reset_object_self", env)
                        elif req == 'all':
                            print("[keys] reset all")
                            env_cfg.event_manager.trigger("reset_all_self", env)
                    except Exception as e:
                        print(f"[keys] reset handling failed: {e}")
                else:
