...
When writing, concatenate multiple images (head, left, right, world) horizontally and write them.
When reading, split the concatenated image back into independent images.
"""
...
SHM_SIZE = 640 * 480 * 3 * 4 + 1024  # support up to 4 images (head + left + right + world) + header buffer
...
class MultiImageWriter:
    ...
    def write_images(self, images: Dict[str, np.ndarray]) -> bool:
        ...
        Args:
            images: the image dictionary, the key is the image name
                    (supports 'head', 'left', 'right', 'world'), the value is the image array
        ...
        try:
            # get the images in order: head, left, right, world (if provided)
            frames_to_concat = []
            image_order = ['head', 'left', 'right', 'world']
...
class MultiImageReader:
    ...
    def read_images(self) -> Optional[Dict[str, np.ndarray]]:
        ...
        try:
            ...
            # split the images
            images = {}
            # support up to 4 named slots for backward compatibility
            image_names = ['head', 'left', 'right', 'world']
