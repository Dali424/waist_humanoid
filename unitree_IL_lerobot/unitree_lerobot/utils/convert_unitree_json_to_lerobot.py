...
class JsonDataset:
    def __init__(
        ...
        data_dirs: Path,
        robot_type: str,
        camera_to_image_key_override: dict[str, str] | None = None,
    ) -> None:
        """
        ...
        # Allow overriding the mapping from raw camera keys (e.g., 'color_0') to
        # dataset image keys (e.g., 'cam_front'). Default to robot config.
        self.camera_to_image_key = (
            camera_to_image_key_override
            if camera_to_image_key_override is not None
            else ROBOT_CONFIGS[robot_type].camera_to_image_key
        )
...
def create_empty_dataset(
    ...
    *,
    ...
    cameras_override: list[str] | None = None,
    image_shape: tuple[int, int, int] | None = None,
) -> LeRobotDataset:
    motors = ROBOT_CONFIGS[robot_type].motors
    cameras = cameras_override if cameras_override is not None else ROBOT_CONFIGS[robot_type].cameras
    ...
    # Default to 480x640 RGB if shape not provided
...
def populate_dataset(
    ...
    json_dataset = JsonDataset(raw_dir, robot_type, camera_to_image_key_override)
...
def json_to_lerobot(
    ...
    *,
    ...
    raw_camera_keys: list[str] | None = None,
):
    if (HF_LEROBOT_HOME / repo_id).exists():
        shutil.rmtree(HF_LEROBOT_HOME / repo_id)

    # Build optional camera mapping override if user specified camera names.
    camera_to_image_key_override = None
    cameras_override = None

    if camera_names is not None:
        cameras_override = camera_names
        if raw_camera_keys is None:
            raw_camera_keys = [f"color_{i}" for i in range(len(camera_names))]
        if len(raw_camera_keys) != len(camera_names):
            raise ValueError(
                f"raw_camera_keys length ({len(raw_camera_keys)}) must match camera_names length ({len(camera_names)})."
            )
        camera_to_image_key_override = dict(zip(raw_camera_keys, camera_names, strict=True))

    # If custom cameras provided, try to infer image shape from the first episode
    inferred_image_shape = None
    if cameras_override is not None:
        try:
            temp_json_ds = JsonDataset(raw_dir, robot_type, camera_to_image_key_override)
            sample = temp_json_ds.get_item(0)
            h, w = sample["data_cfg"]["cam_height"], sample["data_cfg"]["cam_width"]
            inferred_image_shape = (h, w, 3)
        except Exception:
            # Fall back to default shape if inference fails
            inferred_image_shape = None
    ...
    dataset = populate_dataset(
        ...
        raw_dir,
        robot_type=robot_type,
        camera_to_image_key_override=camera_to_image_key_override,
    )
