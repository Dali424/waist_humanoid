...
ARM_CONFIG = {
    # waist_dof: G1_29 has 3 (yaw, roll, pitch); G1_23 has 1 (yaw only)
    "G1_29": {"controller": G1_29_ArmController, "ik_solver": G1_29_ArmIK, "dof": 14, "waist_dof": 3},
    "G1_23": {"controller": G1_23_ArmController, "ik_solver": G1_23_ArmIK, "dof": 14, "waist_dof": 1},
    # Add other arms here
}
...
def setup_image_client(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if getattr(args, "sim", False):
        # Simulation: head is a single view (front_camera); wrist stream packs
        # left_wrist, right_wrist, and world cameras horizontally.
        img_config = {
            "fps": 30,
            ...
            "wrist_camera_image_shape": [480, 640],  # Per-wrist resolution (before concatenation)
            "wrist_camera_id_numbers": [2, 4],
        }
    else:
        # Real robot: head is stereo (1280), wrist stream carries two wrist cameras.
        img_config = {
            "fps": 30,
            "head_camera_type": "opencv",
    ...
    if WRIST and getattr(args, "sim", False):
        # Sim: wrist stream packs [left_wrist, right_wrist, world] horizontally.
        wrist_img_shape = (
            img_config["wrist_camera_image_shape"][0],
            img_config["wrist_camera_image_shape"][1] * 3,
            3,
        )
        ...
        # Real robot: wrist stream packs [left_wrist, right_wrist].
        wrist_img_shape = (
            img_config["wrist_camera_image_shape"][0],
...
def setup_robot_interface(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # Enable waist control if requested
    if hasattr(args, "use_waist") and args.use_waist and hasattr(arm_ctrl, "enable_waist_control"):
    ...
    if is_sim:
        ...
        return {
            ...
            "ee_dof": ee_dof,
            "waist_dof": int(arm_spec.get("waist_dof", 0)),
...
def process_images_and_observations(
    ...
    # Head camera (color_0): single front view in sim; potentially stereo in real.
    ...
    left_wrist_cam = right_wrist_cam = world_cam = None
    if has_wrist_cam and current_wrist_image is not None:
        # Heuristic: if wrist stream width >= 3 * head width, it encodes [left, right, world].
        if wrist_img_shape[1] >= tv_img_shape[1] * 3:
            seg_w = wrist_img_shape[1] // 3
            left_wrist_cam = current_wrist_image[:, 0:seg_w]
            right_wrist_cam = current_wrist_image[:, seg_w : 2 * seg_w]
            world_cam = current_wrist_image[:, 2 * seg_w : 3 * seg_w]
        else:
            # Fallback: only left/right wrists packed.
            seg_w = wrist_img_shape[1] // 2
            left_wrist_cam = current_wrist_image[:, 0:seg_w]
            right_wrist_cam = current_wrist_image[:, seg_w : 2 * seg_w]

    # Provide both camera-named keys and color_N keys to match various datasets/policies.
    # Mapping (by design for Inspire farspawn dataset):
    #   color_0 = head
    #   color_1 = left wrist
    #   color_2 = right wrist
    #   color_3 = world (perspective) view
    raw_observation = {
        # Camera-named keys (for backward compatibility)
        "observation.images.cam_left_high": torch.from_numpy(left_top_cam) if left_top_cam is not None else None,
        "observation.images.cam_right_high": torch.from_numpy(right_top_cam) if is_binocular and right_top_cam is not None else None,
        ...
        # color_i keys used during training
        "observation.images.color_0": torch.from_numpy(left_top_cam) if left_top_cam is not None else None,
        "observation.images.color_1": torch.from_numpy(left_wrist_cam) if left_wrist_cam is not None else None,
        "observation.images.color_2": torch.from_numpy(right_wrist_cam) if right_wrist_cam is not None else None,
        "observation.images.color_3": torch.from_numpy(world_cam) if world_cam is not None else None,
    }
    # Drop keys where we don't have a valid image for this setup.
    observation = {k: v for k, v in raw_observation.items() if v is not None}
