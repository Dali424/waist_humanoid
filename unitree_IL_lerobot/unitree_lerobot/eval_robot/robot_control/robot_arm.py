...
class G1_29_ArmController:
    def __init__(self, motion_mode=False, simulation_mode=False):
        ...
        self.control_waist = False
        self.waist_q_target = None  # np.array([yaw, roll, pitch])
    ...
    def _ctrl_motor_state(self):
        ...
        while True:
            ...
            # Command waist joints if enabled
            if self.control_waist and waist_q_target is not None:
                self.msg.motor_cmd[G1_29_JointIndex.kWaistYaw].q = float(waist_q_target[0])
                self.msg.motor_cmd[G1_29_JointIndex.kWaistYaw].dq = 0
                self.msg.motor_cmd[G1_29_JointIndex.kWaistYaw].tau = 0
                self.msg.motor_cmd[G1_29_JointIndex.kWaistRoll].q = float(waist_q_target[1])
                self.msg.motor_cmd[G1_29_JointIndex.kWaistRoll].dq = 0
                self.msg.motor_cmd[G1_29_JointIndex.kWaistRoll].tau = 0
                self.msg.motor_cmd[G1_29_JointIndex.kWaistPitch].q = float(waist_q_target[2])
                self.msg.motor_cmd[G1_29_JointIndex.kWaistPitch].dq = 0
                self.msg.motor_cmd[G1_29_JointIndex.kWaistPitch].tau = 0
    ...
    def ctrl_arm_and_waist(self, arm_q_target, tauff_target, waist_q_target):
        """Set target for both arms (14) and waist (3)."""
        with self.ctrl_lock:
    ...
    def get_current_waist_q(self):
        """Return current state q of the waist joints (yaw, roll, pitch)."""
        return np.array([
            self.lowstate_buffer.GetData().motor_state[G1_29_JointIndex.kWaistYaw].q,
            self.lowstate_buffer.GetData().motor_state[G1_29_JointIndex.kWaistRoll].q,
            self.lowstate_buffer.GetData().motor_state[G1_29_JointIndex.kWaistPitch].q,
        ])

    def enable_waist_control(self, enable: bool = True):
        """Enable commanding the waist joints with lower gains similar to arm joints."""
        self.control_waist = bool(enable)
        # Set waist gains to low for smoother control
        for jid in [G1_29_JointIndex.kWaistYaw, G1_29_JointIndex.kWaistRoll, G1_29_JointIndex.kWaistPitch]:
            self.msg.motor_cmd[jid].kp = self.kp_low
            self.msg.motor_cmd[jid].kd = self.kd_low
        # Seed waist target with current q to avoid jumps
        self.waist_q_target = np.array([
            self.lowstate_buffer.GetData().motor_state[G1_29_JointIndex.kWaistYaw].q,
            self.lowstate_buffer.GetData().motor_state[G1_29_JointIndex.kWaistRoll].q,
            self.lowstate_buffer.GetData().motor_state[G1_29_JointIndex.kWaistPitch].q,
        ], dtype=float)
...
class G1_23_ArmController:
    def __init__(self, motion_mode=False, simulation_mode=False):
        ...
        self.control_waist = False
        self.waist_yaw_q_target = None  # scalar
    ...
    def _ctrl_motor_state(self):
        ...
        while True:
            ...
            with self.ctrl_lock:
                arm_q_target = self.q_target
                arm_tauff_target = self.tauff_target
                waist_yaw_q_target = self.waist_yaw_q_target
            ...
            # Command waist yaw if enabled
            if self.control_waist and waist_yaw_q_target is not None:
                self.msg.motor_cmd[G1_23_JointIndex.kWaistYaw].q = float(waist_yaw_q_target)
    ...
    def ctrl_arm_and_waist(self, arm_q_target, tauff_target, waist_q_target):
        """Set target for both arms (10) and waist (1 yaw)."""
        with self.ctrl_lock:
            self.q_target = arm_q_target
            self.tauff_target = tauff_target
            self.waist_yaw_q_target = float(waist_q_target[0]) if waist_q_target is not None else None
    ...
    def get_current_waist_q(self):
        """Return current state q of the waist (yaw only for G1_23)."""
        return np.array([self.lowstate_buffer.GetData().motor_state[G1_23_JointIndex.kWaistYaw].q])

    def enable_waist_control(self, enable: bool = True):
        self.control_waist = bool(enable)
        # Set waist yaw gains to low for smoother control
        self.msg.motor_cmd[G1_23_JointIndex.kWaistYaw].kp = self.kp_low
        self.msg.motor_cmd[G1_23_JointIndex.kWaistYaw].kd = self.kd_low
        # Seed target with current state
        self.waist_yaw_q_target = self.lowstate_buffer.GetData().motor_state[G1_23_JointIndex.kWaistYaw].q
