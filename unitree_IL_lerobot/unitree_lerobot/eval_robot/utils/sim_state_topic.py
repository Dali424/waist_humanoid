...
class SharedMemoryManager:
    ...
    def __init__(self, name: str | None = None, size: int = 512):
        ...
        self._closed = False  # guard against double cleanup
    ...
    def cleanup(self):
        ...
        if hasattr(self, "shm") and self.shm:
            try:
                ...
                # Another process may have unlinked already
                logger_mp.debug("[SharedMemoryManager] Shared memory already closed")

            if self.created:
                try:
                    self.shm.unlink()
                except FileNotFoundError:
                    # Ignore if already unlinked elsewhere
                    logger_mp.debug(f"[SharedMemoryManager] Shared memory {self.shm_name} already unlinked")
