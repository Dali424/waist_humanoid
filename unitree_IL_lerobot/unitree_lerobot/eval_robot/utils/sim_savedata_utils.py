...
def process_data_add(
    ...
    if (
        ...
        actions = {
            ...
            "body": {
                "qpos": (
                    action[(arm_dof + 2 * ee_dof) : (arm_dof + 2 * ee_dof + waist_dof)].tolist()
                    if waist_dof > 0
                    else []
                ),
            },
...
class EvalRealConfig:
    ...
    attention_interval: int = 50  # steps between attention saves
    attention_dir: str = "attention"  # subfolder under task_dir to write images
