...
class ACTHierDualQRefineAdapPolicy(PreTrainedPolicy):
    """
    Action Chunking Transformer Policy as per Learning Fine-Grained Bimanual Manipulation with Low-Cost
    Hardware (paper: https://huggingface.co/papers/2304.13705, code: https://github.com/tonyzhaozh/act)
    """

    config_class = ACTHierDualQRefineAdapConfig
    name = "act_hier_dual_qrefine_adap"

    def __init__(
        self,
        config: ACTHierDualQRefineAdapConfig,
...
class ACTTemporalEnsembler:
    ...
    def update(self, actions: Tensor) -> Tensor:
        """Takes a sequence of actions, update the temporal ensemble, and return the next action."""
...
# [NOVELTY] Idea 3 Helper: Adaptive Gating Mechanism
class AdaptiveGate(nn.Module):
    """
    Computes a gating factor alpha based on the current decoder hidden state,
    to adaptively control the flow of context information (e.g., from Waist to Arm).
    """
    ...
    def forward(self, x, context_out):
        # x: Current decoder hidden state (Query side)
        # context_out: Output from Cross-Attention
        alpha = self.gate_net(x)
        return alpha * context_out


class ACT(nn.Module):
    """Action Chunking Transformer: The underlying neural network for ACTHierDualPolicy."""

    def __init__(self, config: ACTHierDualQRefineAdapConfig):
        super().__init__()
        self.config = config
        ...
        # Projects waist latent features to modify the initial queries for the arm decoder.
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        # Pass refined queries (arm_queries) instead of original decoder_queries
        ...
        # Prepare for heads (transpose to B, S, D)
        waist_tokens_t = waist_latent.transpose(0, 1)
        arm_tokens_t = arm_latent.transpose(0, 1)

        waist = self.waist_head(waist_tokens_t)
        arm = self.arm_head(arm_tokens_t)
        hand = self.hand_head(arm_tokens_t)
...
class ACTEncoder(nn.Module):
    ...
    def __init__(self, config: ACTHierDualQRefineAdapConfig, is_vae_encoder: bool = False):
        super().__init__()
...
class ACTArmDualDecoder(nn.Module):
    ...
    def forward(
        ...
        decoder_pos_embed: Tensor | None = None,
        encoder_pos_embed: Tensor | None = None,
        waist_pos_embed: Tensor | None = None,
    ) -> Tensor:
        for layer in self.layers:
            x = layer(
                x,
                ...
                waist_pos_embed=waist_pos_embed,
            )
        if self.norm is not None:
            x = self.norm(x)
        return x
...
class ACTArmDualDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # [NOVELTY] Idea 3: Gated Cross-Attention to Waist Outputs
        if self.pre_norm:
            skip = x
            x = self.norm3(x)
        else:
        ...
        # Calculate context from waist
        waist_context = self.cross_attn_waist(
            query=self.maybe_add_pos_embed(x, decoder_pos_embed),
            key=self.maybe_add_pos_embed(waist_out, waist_pos_embed),
            value=waist_out,
        )[0]
