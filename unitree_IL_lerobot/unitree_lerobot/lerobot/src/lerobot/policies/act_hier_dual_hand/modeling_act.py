...
class ACTHierDualHandPolicy(PreTrainedPolicy):
    config_class = ACTHierDualHandConfig
    name = "act_hier_dual_hand"

    def __init__(self, config: ACTHierDualHandConfig, dataset_stats: dict[str, dict[str, Tensor]] | None = None):
    ...
    def update(self, actions: Tensor) -> Tensor:
        ...
        if self.ensembled_actions is None:
            ...
            self.ensembled_actions = torch.cat(
                [self.ensembled_actions, actions[:, -1:] * self.ensemble_weights[0]], dim=1
            )
...
class ACT(nn.Module):
    def __init__(self, config: ACTHierDualHandConfig):
        super().__init__()
        self.config = config
        ...
        self.waist_arm_decoder = ACTDecoder(config)
        self.hand_decoder = ACTHandDualDecoder(config)
    ...
    def _reset_parameters(self):
        for p in chain(self.encoder.parameters(), self.waist_arm_decoder.parameters(), self.hand_decoder.parameters()):
            if p.dim() > 1:
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        waist_arm_latent = self.waist_arm_decoder(
            decoder_queries,
            encoder_out,
            encoder_pos_embed=encoder_in_pos_embed,
            decoder_pos_embed=decoder_pos,
        )
        waist_arm_tokens = waist_arm_latent
        waist_arm_latent = waist_arm_latent.transpose(0, 1)
        waist = self.waist_head(waist_arm_latent)
        arm = self.arm_head(waist_arm_latent)

        waist_pos = decoder_pos.expand(-1, waist_arm_tokens.shape[1], -1)
        hand_latent = self.hand_decoder(
            decoder_queries,
            encoder_out,
            waist_arm_tokens,
            encoder_pos_embed=encoder_in_pos_embed,
        ...
        hand = self.hand_head(hand_latent)

        action_dim = self.config.action_feature.shape[0]
        actions = torch.zeros(
            hand_latent.shape[0], hand_latent.shape[1], action_dim, device=hand_latent.device, dtype=hand_latent.dtype
        )
...
class ACTEncoder(nn.Module):
    def __init__(self, config: ACTHierDualHandConfig, is_vae_encoder: bool = False):
        super().__init__()
...
class ACTHandDualDecoder(nn.Module):
    def __init__(self, config: ACTHierDualHandConfig):
        super().__init__()
        self.layers = nn.ModuleList([ACTHandDualDecoderLayer(config) for _ in range(config.n_decoder_layers)])
        self.norm = nn.LayerNorm(config.dim_model)
    ...
    def forward(
        ...
        x: Tensor,
        encoder_out: Tensor,
        waist_arm_out: Tensor,
        decoder_pos_embed: Tensor | None = None,
        encoder_pos_embed: Tensor | None = None,
        waist_arm_pos_embed: Tensor | None = None,
    ) -> Tensor:
        for layer in self.layers:
            x = layer(
                x,
                encoder_out=encoder_out,
                waist_arm_out=waist_arm_out,
                decoder_pos_embed=decoder_pos_embed,
                encoder_pos_embed=encoder_pos_embed,
                waist_arm_pos_embed=waist_arm_pos_embed,
            )
        if self.norm is not None:
            x = self.norm(x)
        return x
...
class ACTHandDualDecoderLayer(nn.Module):
    def __init__(self, config: ACTHierDualHandConfig):
        super().__init__()
        self.self_attn = nn.MultiheadAttention(config.dim_model, config.n_heads, dropout=config.dropout)
        self.cross_attn_encoder = nn.MultiheadAttention(config.dim_model, config.n_heads, dropout=config.dropout)
        self.cross_attn_waist_arm = nn.MultiheadAttention(config.dim_model, config.n_heads, dropout=config.dropout)
    ...
    def forward(
        ...
        if self.pre_norm:
            ...
            x = self.norm_mid(x)
            skip = x
        x = self.cross_attn_waist_arm(
            query=self.maybe_add_pos_embed(x, decoder_pos_embed),
            key=self.maybe_add_pos_embed(waist_arm_out, waist_arm_pos_embed),
            value=waist_arm_out,
        )[0]
...
class ACTSinusoidalPositionEmbedding2d(nn.Module):
    ...
    def forward(self, tensor_list: Tensor) -> Tensor:
        B, _, H, W = tensor_list.shape
        mask = torch.zeros(B, H, W, device=tensor_list.device, dtype=torch.bool)
        not_mask = ~mask
        y_embed = not_mask.cumsum(1, dtype=tensor_list.dtype)
        x_embed = not_mask.cumsum(2, dtype=tensor_list.dtype)
        eps = 1e-6
        y_embed = y_embed / (y_embed[:, -1:, :] + eps) * self.scale
        x_embed = x_embed / (x_embed[:, :, -1:] + eps) * self.scale

        dim_t = torch.arange(self.dimension, dtype=tensor_list.dtype, device=tensor_list.device)
        dim_t = 10000 ** (2 * (dim_t // 2) / self.dimension)
