...
class ACTHier3AdapPolicy(PreTrainedPolicy):
    """
    Action Chunking Transformer Policy as per Learning Fine-Grained Bimanual Manipulation with Low-Cost
    Hardware (paper: https://huggingface.co/papers/2304.13705, code: https://github.com/tonyzhaozh/act)
    """

    config_class = ACTHier3AdapConfig
    name = "act_hier3_adap"

    def __init__(
        self,
        config: ACTHier3AdapConfig,
...
class ACT(nn.Module):
    """Action Chunking Transformer: The underlying neural network for ACTHier3Policy.
    ...
    def __init__(self, config: ACTHier3AdapConfig):
    ...
    def _reset_parameters(self):
        ...
        for module in (*self.waist_gating.gate_net, *self.arm_gating.gate_net):
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        # Arm decoder: keep fresh queries, but extend encoder memory with gated waist tokens.
        waist_tokens_b = waist_tokens.transpose(0, 1)
        queries_b = decoder_queries.transpose(0, 1)
        gated_waist_tokens = self.waist_gating(queries_b, waist_tokens_b).transpose(0, 1)
        ...
        # Hand decoder: condition on encoder, gated waist, and gated arm latents.
        arm_tokens_b = arm_tokens.transpose(0, 1)
        gated_arm_tokens = self.arm_gating(queries_b, arm_tokens_b).transpose(0, 1)
        hand_memory = torch.cat([encoder_out, gated_waist_tokens, gated_arm_tokens], dim=0)
        hand_memory_pos = torch.cat(
            [
                encoder_in_pos_embed.expand(-1, gated_waist_tokens.shape[1], -1),
                torch.zeros_like(gated_waist_tokens),
                torch.zeros_like(gated_arm_tokens),
            ],
            dim=0,
        )
...
class ACTEncoder(nn.Module):
    ...
    def __init__(self, config: ACTHier3AdapConfig, is_vae_encoder: bool = False):
        super().__init__()
