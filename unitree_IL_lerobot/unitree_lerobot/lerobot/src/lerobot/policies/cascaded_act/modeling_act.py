...
class ACT(nn.Module):
    ...
    def __init__(self, config: CascadedACTConfig):
        ...
        self.ee_decoder = ACTDecoder(config)
        ...
        self.arm_head = nn.Linear(config.dim_model, 14)
        self.ee_head = nn.Linear(config.dim_model, 12)
        self.waist_token_proj = nn.Linear(3, config.dim_model)
        self.arm_token_proj = nn.Linear(14, config.dim_model)

        self._reset_parameters()

    def _reset_parameters(self):
        """Xavier-uniform initialization of the transformer parameters as in the original code."""
        modules = [self.encoder, self.waist_decoder, self.arm_decoder, self.ee_decoder]
        for m in modules:
            for p in m.parameters():
                if p.dim() > 1:
                    nn.init.xavier_uniform_(p)
        for head in [self.waist_head, self.arm_head, self.ee_head, self.waist_token_proj, self.arm_token_proj]:
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, dict]:
        ...
        waist_tokens = self.waist_token_proj(pred_body).transpose(0, 1)  # (S,B,D)
        ...
        arm_tokens = self.arm_token_proj(pred_arms).transpose(0, 1)  # (S,B,D)
        ...
        left_arm = pred_arms[:, :, :7]
        right_arm = pred_arms[:, :, 7:14]
        left_ee = pred_ee[:, :, :6]
        right_ee = pred_ee[:, :, 6:12]
        ...
        if self.training and ACTION in batch and "action_is_pad" in batch:
            ...
            l_arm = masked_mse(pred_arms, torch.cat([gt_left_arm, gt_right_arm], dim=-1))
            l_ee = masked_mse(pred_ee, torch.cat([gt_left_ee, gt_right_ee], dim=-1))
