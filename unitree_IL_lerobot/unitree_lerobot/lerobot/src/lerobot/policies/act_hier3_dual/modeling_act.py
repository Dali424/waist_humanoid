...
class ACTHier3DualPolicy(PreTrainedPolicy):
    """
    Action Chunking Transformer Policy as per Learning Fine-Grained Bimanual Manipulation with Low-Cost
    Hardware (paper: https://huggingface.co/papers/2304.13705, code: https://github.com/tonyzhaozh/act)
    """

    config_class = ACTHier3DualConfig
    name = "act_hier3_dual"

    def __init__(
        self,
        config: ACTHier3DualConfig,
...
class ACT(nn.Module):
    """Action Chunking Transformer: The underlying neural network for ACTHier3DualPolicy.
    ...
    def __init__(self, config: ACTHier3DualConfig):
        ...
        # Dual cross-attn decoders: encoder + previous decoder outputs.
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        # Arm decoder: dual cross-attn on encoder then waist tokens.
        waist_pos = decoder_pos.expand(-1, waist_tokens.shape[1], -1)
        arm_dec_out = self.arm_decoder(
            decoder_queries,
            encoder_out,
            prev_out=waist_tokens,
            encoder_pos_embed=encoder_in_pos_embed,
            prev_pos_embed=waist_pos,
            decoder_pos_embed=decoder_pos,
        )
        ...
        # Hand decoder: dual cross-attn on encoder then waist+arm tokens.
        arm_pos = decoder_pos.expand(-1, arm_tokens.shape[1], -1)
        hand_prev_out = torch.cat([waist_tokens, arm_tokens], dim=0)
        hand_prev_pos = torch.cat([waist_pos, arm_pos], dim=0)
        hand_dec_out = self.hand_decoder(
            decoder_queries,
            encoder_out,
            prev_out=hand_prev_out,
            encoder_pos_embed=encoder_in_pos_embed,
            prev_pos_embed=hand_prev_pos,
            decoder_pos_embed=decoder_pos,
        )
...
class ACTDualDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Optional cross-attention to previous decoder outputs
        ...
        if self.pre_norm:
            ...
            x = self.norm3(x) if prev_out is not None else self.norm_mid(x)
            skip = x
